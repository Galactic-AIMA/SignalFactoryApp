...
export async function POST(req: NextRequest) {
  try {
    ...
    const { desde, hasta, idioma, estilo, efecto } = schema.parse(body);
    ...
    renderQueue.renderBatch(desde, hasta, { idioma, estilo, efecto }).catch(() => {});

    const videos = idioma === "ambos" ? (hasta - desde + 1) * 2 : hasta - desde + 1;
    return NextResponse.json({
      ok: true,
      mensaje: `Lote ${desde}-${hasta} iniciado — ${hasta - desde + 1} números × ${idioma === "ambos" ? "2 idiomas" : idioma.toUpperCase()} = ${videos} videos`,
    });
  } catch (err) {
